...
def RecallPrecisionFScore(y_test, pred):
    ...
    return (
        float("{0:.2f}".format(precision * 100)),
        float("{0:.2f}".format(recall * 100)),
        float("{0:.2f}".format(f1 * 100)),
    )
...
def embedding_feature_based_combination(df_train, df_test, c, ux, uy, cx, cy):
    ...
    ft = gensim.models.KeyedVectors.load_word2vec_format(
        os.path.join(MODEL_BASE_PATH, "cc.bn.300.vec"), binary=False
    )
...
def feature_based(df_train, df_test, c, x1, y1, gram):
    ...
    tfidf_vect_ngram = TfidfVectorizer(
        analyzer=gram, ngram_range=(x1, y1), tokenizer=lambda x: x.split()
    )
...
path_parent = os.path.dirname(os.getcwd())
os.chdir(path_parent)
df_train = pd.read_csv(os.path.join(DATA_BASE_PATH, "Final_Train.csv"))
df_val = pd.read_csv(os.path.join(DATA_BASE_PATH, "Final_Val.csv"))
df_test = pd.read_csv(os.path.join(DATA_BASE_PATH, "Final_Test.csv"))
...
#         embedding_feature_based_combination(
#             df_train, df_test, c=1, ux=1, uy=2, cx=2, cy=5
#         )

#     elif model_name == "U+B+T+C2+C3+C4+C5+E":
#         embedding_feature_based_combination(
#             df_train, df_test, c=1, ux=1, uy=3, cx=2, cy=5
#         )
